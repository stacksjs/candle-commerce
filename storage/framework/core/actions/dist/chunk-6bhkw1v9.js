...
var resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./chunk-v06xkn1v.js");
      const { fromContainerMetadata } = await import("./chunk-d9snajpc.js");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("./chunk-d9snajpc.js");
...
var resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./chunk-xcv10fc1.js");
...
var resolveProcessCredentials = async (options, profile) => import("./chunk-qb152aqd.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
...
var resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./chunk-zga33sx2.js");
  return fromSSO({
    profile,
...
var resolveWebIdentityCredentials = async (profile, options) => import("./chunk-x9d8pe5a.js").then(({ fromTokenFile }) => fromTokenFile({
