...
var resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./chunk-bnyjwra2.js");
...
var resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./chunk-7sg8rd4r.js");
...
var resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./chunk-1g272y0k.js");
  return fromSSO({
    profile,
...
var resolveWebIdentityCredentials = async (profile, options) => import("./chunk-0dgzz5w0.js").then(({ fromTokenFile }) => fromTokenFile({
