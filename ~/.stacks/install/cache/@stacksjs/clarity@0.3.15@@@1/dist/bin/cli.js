...
function getProjectRoot(filePath, options = {}) {
  let path = process3.cwd();
  while (path.includes("storage"))
    path = resolve2(path, "..");
  const finalPath = resolve2(path, filePath || "");
  if (options?.relative)
    return relative(process3.cwd(), finalPath);
  return finalPath;
}
var defaultLogDirectory = process3.env.CLARITY_LOG_DIR || join(getProjectRoot(), "logs");
var defaultConfig = {
  level: "info",
  defaultName: "clarity",
  timestamp: true,
...
function isBrowserProcess() {
  if (process4.env.NODE_ENV === "test" || process4.env.BUN_ENV === "test") {
    return false;
  }
...
async function isServerProcess() {
  ...
  if (typeof process4 !== "undefined") {
    const type = process4.type;
    if (type === "renderer" || type === "worker") {
      return false;
    }
    return !!(process4.versions && (process4.versions.node || process4.versions.bun));
  }
  return false;
}
  ...
  async getMetadata(isServer) {
    if (isServer) {
      ...
      return {
        pid: process5.pid,
        hostname: hostname(),
        environment: process5.env.NODE_ENV || "development",
        platform: process5.platform,
        version: process5.version
      };
    }
    return {
      userAgent: navigator.userAgent,
      hostname: window.location.hostname || "browser",
      environment: process5.env.NODE_ENV || process5.env.BUN_ENV || "development",
      viewport: {
...
class Logger {
  ...
  constructor(name, options = {}) {
    ...
    this.environment = options.environment ?? process6.env.APP_ENV ?? "local";
  ...
  formatConsoleMessage(parts) {
    ...
    const terminalWidth = process6.stdout.columns || 120;
    let mainPart = "";
  ...
  async prompt(message) {
    ...
    return new Promise((resolve3) => {
      ...
      const onData = (data) => {
        ...
        process6.stdin.removeListener("data", onData);
        try {
          if (typeof process6.stdin.setRawMode === "function") {
      ...
      process6.stdin.once("data", onData);
    });
  }
  setFancy(enabled) {
    this.fancy = enabled;
...
`.replace(this.ANSI_PATTERN, "");
  ...
  progress(total, initialMessage = "") {
    ...
    const interrupt = (interruptMessage, level = "info") => {
      if (!this.activeProgressBar || !this.enabled || !this.fancy || isBrowserProcess())
        return;
      process6.stdout.write(`${"\r".padEnd(process6.stdout.columns || 80)}\r`);
  ...
  renderProgressBar(barState, isFinished = false) {
    if (!this.enabled || !this.fancy || isBrowserProcess() || !process6.stdout.isTTY)
      return;
    ...
    const terminalWidth = process6.stdout.columns || 80;
    ...
    process6.stdout.write(barState.lastRenderedLine);
    if (isFinished) {
...
var CONFIG_FILE = join3(homedir(), ".clarity", "config.json");
...
class ConfigManagerImpl {
  ...
  async save() {
    await writeFile2(CONFIG_FILE, JSON.stringify(this.config, null, 2));
  }
}
var configManager = new ConfigManagerImpl;

// src/cli/config-handler.ts
var CONFIG_FILE2 = join4(homedir2(), ".clarity", "config.json");
async function loadConfig3() {
  try {
    const content = await readFile2(CONFIG_FILE2, "utf8");
    return JSON.parse(content);
  } catch {
    return {};
  }
}
async function saveConfig(config3) {
  try {
    await writeFile3(CONFIG_FILE2, JSON.stringify(config3, null, 2));
  } catch (error) {
...
async function handleLog(logger2, message, options) {
  ...
  const logDir = logger2.getLogDirectory();
  console.error("Debug: Creating log directory:", logDir);
  try {
    await mkdir2(logDir, { recursive: true, mode: 493 });
  ...
  try {
    switch (validLevel) {
      case "debug":
        await logger2.debug(message, metadata);
        break;
      case "info":
        await logger2.info(message, metadata);
        break;
      case "success":
        await logger2.success(message, metadata);
        break;
      case "warning":
        await logger2.warn(message, metadata);
        break;
      case "error":
        await logger2.error(message, metadata);
        break;
    }
...
async function handleExport(logger2, options) {
  const { format: _format, output, level: _level, name: _name, start: _start, end: _end, compress } = options;
  try {
    const stream = logger2.createReadStream();
    const writeStream = output ? createWriteStream2(output) : process8.stdout;
    if (compress)
      await pipeline2(stream, createGzip2(), writeStream);
    else
...
async function handleTail(logger2, options) {
...
async function handleSearch(logger2, pattern, options) {
...
async function handleClear(logger2, options) {
  const { level: _level, name, before } = options;
  try {
    if (!options.force) {
      const readline = createInterface({
        input: process8.stdin,
        output: process8.stdout
      });
      const answer = await new Promise((resolve3) => {
        readline.question("Are you sure you want to clear logs? [y/N] ", resolve3);
      });
      readline.close();
...
async function handleConfig(action, options, handlers) {
  const { key, value, format: format2 = "json" } = options;
  try {
    const config3 = await handlers.load();
    switch (action) {
      case "get":
        if (!key) {
      ...
      case "set":
        ...
        config3[key] = value;
        await handlers.save(config3);
        console.error(`Set ${key}=${value}`);
        break;
      case "list":
        if (format2 === "json")
          console.error(JSON.stringify(config3, null, 2));
        else
          console.error($stringify(config3));
        break;
      case "reset":
        await handlers.save({});
...
async function createCli(options = {
  logDirectory: "logs"
}) {
  const cli = cac("clarity");
  const logger2 = new Logger("cli", {
    logDirectory: options.logDirectory,
    level: "debug"
  });
  cli.command("watch", "Monitor logs in real-time").option("--level <level>", "Filter by log level").option("--name <pattern>", "Filter by logger name").option("--json", "Output in JSON format").option("--timestamp", "Include timestamps").option("--color", "Enable colored output").option("--verbose", "Show detailed information").action(async (options2) => {
    await handleWatch(logger2, options2);
  });
  cli.command("log <message>", "Send one-off log messages").option("--level <level>", "Log level", { default: "info" }).option("--name <n>", "Logger name").option("--json", "Use JSON format").option("--meta <json>", "Add metadata as JSON").action(async (message, options2) => {
    console.error("Debug: CLI log command called with:", { message, options: options2 });
    await handleLog(logger2, message, options2);
  });
  cli.command("export", "Export logs to a file").option("--format <format>", "Output format (json|text|csv)").option("--output <file>", "Output file path").option("--level <level>", "Filter by log level").option("--name <pattern>", "Filter by logger name").option("--start <date>", "Start date (ISO format)").option("--end <date>", "End date (ISO format)").option("--compress", "Compress output file").action(async (options2) => {
    await handleExport(logger2, options2);
  });
  cli.command("tail", "View and follow log output").option("--lines <n>", "Number of lines", { default: 10 }).option("--follow", "Follow log output").option("--level <level>", "Filter by log level").option("--name <pattern>", "Filter by logger name").option("--color", "Enable colored output").action(async (options2) => {
    await handleTail(logger2, options2);
  });
  cli.command("search <pattern>", "Search through logs").option("--level <level>", "Filter by log level").option("--name <pattern>", "Filter by logger name").option("--start <date>", "Start date").option("--end <date>", "End date").option("--case-sensitive", "Enable case-sensitive search").option("--context <n>", "Show n lines of context").action(async (pattern, options2) => {
    await handleSearch(logger2, pattern, options2);
  });
  cli.command("clear", "Clear log history").option("--level <level>", "Clear specific log level").option("--name <pattern>", "Clear specific logger").option("--before <date>", "Clear logs before date").option("--dry-run", "Show what would be deleted").option("--force", "Skip confirmation").action(async (options2) => {
    await handleClear(logger2, options2);
  });
  cli.command("config <action>", "Manage configuration").option("--key <key>", "Configuration key").option("--value <value>", "Configuration value").option("--format <fmt>", "Output format (json|yaml)").action(async (action, options2) => {
    await handleConfig(action, options2, { load: loadConfig3, save: saveConfig });
  });
  cli.option("--config <file>", "Use specific config file").option("--quiet", "Suppress output").option("--debug", "Enable debug output").option("--no-color", "Disable colored output").help().version(version);
  return cli;
}
