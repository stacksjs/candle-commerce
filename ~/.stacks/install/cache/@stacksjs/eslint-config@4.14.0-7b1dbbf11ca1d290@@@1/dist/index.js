...
async function formatters(options = {}, stylistic$1 = {}) {
	...
	const pluginFormat = await interopDefault(import("./dist-EpIIKznp.js"));
	const configs$1 = [{
...
async function loadAutoImports$1() {
	const globals$1 = {};
	const paths = ["../../../../storage/framework/browser-auto-imports.json", "../../../../storage/framework/server-auto-imports.json"];
	for (const path$1 of paths) if (existsSync(join(process.cwd(), path$1))) try {
		const { globals: fileGlobals } = await import(path$1);
		Object.assign(globals$1, fileGlobals);
	} catch (error) {
		console.debug(`Failed to load auto-imports from ${path$1}:`, error);
	}
	return globals$1;
}
async function javascript(options = {}) {
	const { isInEditor = false, overrides = {} } = options;
	const autoImportGlobals = await loadAutoImports$1();
	return [{
		languageOptions: {
			ecmaVersion: 2022,
			globals: {
...
async function unocss(options = {}) {
	...
	await ensurePackages(["@stacksjs/eslint-plugin"]);
	const [pluginUnoCSS] = await Promise.all([interopDefault(import("@stacksjs/eslint-plugin"))]);
	return [{
		name: "antfu/unocss",
...
*/
function stacks(options = {}, ...userConfigs) {
	const { autoRenamePlugins = true, componentExts = [], gitignore: enableGitignore = true, jsx: enableJsx = true, pnpm: enableCatalogs = false, regexp: enableRegexp = true, typescript: enableTypeScript = isPackageExists("typescript"), unicorn: enableUnicorn = true, unocss: enableUnoCSS = false, vue: enableVue = VuePackages.some((i) => isPackageExists(i)) } = options;
	let isInEditor = options.isInEditor;
	if (isInEditor == null) {
		isInEditor = isInEditorEnv();
		if (isInEditor) console.log("[@stacksjs/eslint-config] Detected running in editor, some rules are disabled.");
	}
	...
	if ("files" in options) throw new Error("[@stacksjs/eslint-config] The first argument should not contain the \"files\" property as the options are supposed to be global. Place it in the second or later config instead.");
...
//#endregion
//#region src/index.ts
var src_default = stacks;

//#endregion
export { GLOB_ALL_SRC, GLOB_CSS, GLOB_EXCLUDE, GLOB_GRAPHQL, GLOB_HTML, GLOB_JS, GLOB_JSON, GLOB_JSON5, GLOB_JSONC, GLOB_JSX, GLOB_LESS, GLOB_MARKDOWN, GLOB_MARKDOWN_CODE, GLOB_MARKDOWN_IN_MARKDOWN, GLOB_POSTCSS, GLOB_SCSS, GLOB_SRC, GLOB_SRC_EXT, GLOB_STYLE, GLOB_SVG, GLOB_TESTS, GLOB_TOML, GLOB_TS, GLOB_TSX, GLOB_VUE, GLOB_XML, GLOB_YAML, StylisticConfigDefaults, combine, command, comments, src_default as default, defaultPluginRenaming, disables, ensurePackages, formatters, getOverrides, ignores, imports, interopDefault, isInEditorEnv, isInGitHooksOrLintStaged, isPackageInScope, javascript, jsdoc, jsonc, jsx, markdown, node, parserPlain, perfectionist, pnpm, regexp, renamePluginInConfigs, renameRules, resolveSubOptions, sortPackageJson, sortTsconfig, stacks, stylistic, test, toArray, toml, typescript, unicorn, unocss, vue, yaml };