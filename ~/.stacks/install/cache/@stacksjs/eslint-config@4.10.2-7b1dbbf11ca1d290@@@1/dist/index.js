...
var isCwdInScope = isPackageExists("@stacksjs/eslint-config");
var parserPlain = {
  meta: {
    name: "parser-plain"
  },
...
async function formatters(options = {}, stylistic2 = {}) {
  ...
  const pluginFormat = await interopDefault(import("./dist-B6YJHPMI.js"));
  const configs2 = [
    {
...
async function loadAutoImports() {
  ...
  for (const path of paths) {
    if (existsSync(join(process2.cwd(), path))) {
      try {
        const { globals: fileGlobals } = await import(path);
        Object.assign(globals2, fileGlobals);
      } catch (error) {
...
async function loadAutoImports2() {
  const globals2 = {};
  const paths = [
    "../../../../storage/framework/browser-auto-imports.json",
    "../../../../storage/framework/server-auto-imports.json"
  ...
  for (const path of paths) {
    if (existsSync2(join2(process3.cwd(), path))) {
      try {
        const { globals: fileGlobals } = await import(path);
        Object.assign(globals2, fileGlobals);
      } catch (error) {
...
async function typescript(options = {}) {
  ...
  const autoImportGlobals = await loadAutoImports2();
