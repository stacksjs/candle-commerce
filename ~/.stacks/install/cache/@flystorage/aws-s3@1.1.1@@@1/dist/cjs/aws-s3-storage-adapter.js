...
exports.writeOptionResolvers = exports.AwsS3FileStorage = exports.AwsS3StorageAdapter = exports.HostStyleAwsPublicUrlGenerator = exports.DefaultAwsPublicUrlGenerator = void 0;
...
class DefaultAwsPublicUrlGenerator {
...
exports.DefaultAwsPublicUrlGenerator = DefaultAwsPublicUrlGenerator;
/**
 * BC extension
 */
class HostStyleAwsPublicUrlGenerator extends DefaultAwsPublicUrlGenerator {
}
exports.HostStyleAwsPublicUrlGenerator = HostStyleAwsPublicUrlGenerator;
/**
...
class AwsS3StorageAdapter {
    ...
    constructor(client, options, publicUrlGenerator = new DefaultAwsPublicUrlGenerator(), timestampResolver = () => Date.now()) {
        ...
        this.prefixer = new file_storage_1.PathPrefixer(options.prefix ?? '', '/', (...paths) => {
            const path = (0, node_path_1.join)(...paths);
            if (path === "." || path === "/") {
    ...
    async copyFile(from, to, options) {
        ...
        await this.client.send(new client_s3_1.CopyObjectCommand({
            ...
            CopySource: (0, node_path_1.join)('/', this.options.bucket, encodePath(this.prefixer.prefixFilePath(from))),
    ...
    async prepareUpload(path, options) {
        maybeAbort(options.abortSignal);
        const expiry = (0, file_storage_1.normalizeExpiryToMilliseconds)(options.expiresAt);
        ...
        const url = await (0, s3_request_presigner_1.getSignedUrl)(this.client, new client_s3_1.PutObjectCommand(putObjectParams), {
    ...
    async temporaryUrl(path, options) {
        ...
        return await (0, s3_request_presigner_1.getSignedUrl)(this.client, new client_s3_1.GetObjectCommand(getObjectParams), {
            expiresIn: Math.floor((expiry - now) / 1000),
        });
    }
    ...
    async visibility(path, options) {
        maybeAbort(options.abortSignal);
        const response = await this.client.send(new client_s3_1.GetObjectAclCommand({
            Bucket: this.options.bucket,
            Key: this.prefixer.prefixFilePath(path),
        }), {
            abortSignal: options.abortSignal,
        ...
        return publicRead ? file_storage_1.Visibility.PUBLIC : file_storage_1.Visibility.PRIVATE;
    }
    ...
    async read(path, options) {
        ...
        let response;
        try {
            response = await this.client.send(new client_s3_1.GetObjectCommand({
                Bucket: this.options.bucket,
                Key: this.prefixer.prefixFilePath(path),
            }), {
                abortSignal: options.abortSignal,
            ...
            if (err instanceof client_s3_1.S3ServiceException && err.$metadata.httpStatusCode === 404) {
                throw file_storage_1.FileWasNotFound.atLocation(path, {
        ...
        if (response.Body instanceof stream_1.Readable || response.Body instanceof ReadableStream) {
    ...
    async deleteDirectory(path) {
        ...
        const flush = async (keys) => this.client.send(new client_s3_1.DeleteObjectsCommand({
            Bucket: this.options.bucket,
            Delete: {
                Objects: keys,
            },
    ...
    async write(path, contents, options) {
        ...
        for (const option of Object.keys(options)) {
            if (isWriteOptionKey(option)) {
                const resolver = exports.writeOptionResolvers[option];
    ...
    async changeVisibility(path, visibility, options) {
        maybeAbort(options.abortSignal);
        await this.client.send(new client_s3_1.PutObjectAclCommand({
            Bucket: this.options.bucket,
            Key: this.prefixer.prefixFilePath(path),
    ...
    async lookupMimeTypeFromStream(path, options) {
        const [mimetype, stream] = await (0, stream_mime_type_1.resolveMimeType)(path, stream_1.Readable.from(await this.read(path, options)));
        await (0, file_storage_1.closeReadable)(stream);
        return mimetype;
    }
}
exports.AwsS3StorageAdapter = AwsS3StorageAdapter;
/**
 * BC export
 *
 * @deprecated
 */
class AwsS3FileStorage extends AwsS3StorageAdapter {
}
exports.AwsS3FileStorage = AwsS3FileStorage;
function isWriteOptionKey(key) {
    return Object.hasOwn(exports.writeOptionResolvers, key);
}
exports.writeOptionResolvers = {
