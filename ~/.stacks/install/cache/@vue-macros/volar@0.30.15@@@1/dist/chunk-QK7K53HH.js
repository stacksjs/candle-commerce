...
function getEmitStrings(options) {
  ...
  function walkNode(node, defaultName = "") {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && _common.DEFINE_EMIT === node.expression.escapedText) {
      ...
      if (!name) return;
      const type = _optionalChain([node, 'access', _ => _.typeArguments, 'optionalAccess', _2 => _2.length]) === 1 ? ts.isFunctionTypeNode(node.typeArguments[0]) ? `Parameters<${_chunk6LLWWLOSjs.getText.call(void 0, node.typeArguments[0], options)}>` : _chunk6LLWWLOSjs.getText.call(void 0, node.typeArguments[0], options) : "[]";
    ...
    if (ts.isExpressionStatement(node)) {
      ...
      ts.forEachChild(node.declarationList, (decl) => {
        if (ts.isVariableDeclaration(decl) && decl.initializer && ts.isIdentifier(decl.name)) {
          walkNode(decl.initializer, _chunk6LLWWLOSjs.getText.call(void 0, decl.name, options));
        }
      });
    }
  });
...
var plugin = (ctx, options = {}) => {
  ...
  return {
    ...
    resolveEmbeddedCode(fileName, sfc, embeddedFile) {
      ...
      _chunk6LLWWLOSjs.addEmits.call(void 0, embeddedFile.content, emitStrings, ctx.vueCompilerOptions.lib);
    }
  };
};
