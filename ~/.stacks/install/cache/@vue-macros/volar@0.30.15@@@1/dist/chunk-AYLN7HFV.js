...
function transformDefineProp({
  ...
  _chunk6LLWWLOSjs.addProps.call(void 0, 
    codes,
...
function getDefineProp(ts, sfc, edition) {
  ...
  function visitNode(node, parent, isReactivityTransform = false) {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && [_common.DEFINE_PROP, _common.DEFINE_PROP_DOLLAR].includes(node.expression.escapedText)) {
      if (edition === "kevinEdition") {
        const type = _optionalChain([node, 'access', _ => _.typeArguments, 'optionalAccess', _2 => _2.length]) ? _chunk6LLWWLOSjs.getText.call(void 0, node.typeArguments[0], { ts, sfc }) : void 0;
        const name = node.arguments[0] && ts.isStringLiteral(node.arguments[0]) ? node.arguments[0].text : ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name) ? _chunk6LLWWLOSjs.getText.call(void 0, parent.name, { ts, sfc }) : void 0;
        const prop = ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name) ? _chunk6LLWWLOSjs.getText.call(void 0, parent.name, { ts, sfc }) : void 0;
        ...
        if (optionArg) {
          for (const property of optionArg.properties) {
            if (ts.isPropertyAssignment(property) && ts.isIdentifier(property.name)) {
              if (_chunk6LLWWLOSjs.getText.call(void 0, property.name, { ts, sfc }) === "required" && property.initializer.kind === ts.SyntaxKind.TrueKeyword)
                required = true;
              if (ts.isIdentifier(property.name) && _chunk6LLWWLOSjs.getText.call(void 0, property.name, { ts, sfc }) === "default")
                defaultValue = _chunk6LLWWLOSjs.getText.call(void 0, property.initializer, { ts, sfc });
            }
          }
        }
        defineProps.push({
        ...
        const name = ts.isIdentifier(parent.name) ? _chunk6LLWWLOSjs.getText.call(void 0, parent.name, { ts, sfc }) : void 0;
        defineProps.push({
          name,
          prop: name,
          defaultValue: node.arguments.length > 0 ? _chunk6LLWWLOSjs.getText.call(void 0, node.arguments[0], { ts, sfc }) : void 0,
          type: _optionalChain([node, 'access', _3 => _3.typeArguments, 'optionalAccess', _4 => _4.length]) ? _chunk6LLWWLOSjs.getText.call(void 0, node.typeArguments[0], { ts, sfc }) : void 0,
