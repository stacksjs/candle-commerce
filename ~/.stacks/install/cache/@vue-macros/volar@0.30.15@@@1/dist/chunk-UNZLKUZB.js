...
function transform(options) {
  ...
  for (const stmt of sfc.scriptSetup.ast.statements) {
    if (ts.isExportDeclaration(stmt) && stmt.exportClause) {
      const start = _chunk6LLWWLOSjs.getStart.call(void 0, stmt, options);
      const end = stmt.end;
      if (ts.isNamedExports(stmt.exportClause)) {
        ...
        stmt.exportClause.elements.forEach((element) => {
          ...
          exportMap.set(
            [
              _chunk6LLWWLOSjs.getText.call(void 0, propertyName, options),
              "scriptSetup",
              _chunk6LLWWLOSjs.getStart.call(void 0, propertyName, options),
              _languagecore.allCodeFeatures
            ],
            [
              _chunk6LLWWLOSjs.getText.call(void 0, name, options),
              "scriptSetup",
              _chunk6LLWWLOSjs.getStart.call(void 0, name, options),
              _languagecore.allCodeFeatures
            ]
          );
          exposed[_chunk6LLWWLOSjs.getText.call(void 0, name, options)] = _chunk6LLWWLOSjs.getText.call(void 0, propertyName, options);
        });
        ...
        _mugglestring.replaceSourceRange.call(void 0, 
          ...
          `;[${_chunk6LLWWLOSjs.getText.call(void 0, stmt.exportClause.name, options)}];`
        );
      }
      ...
      if (ts.isVariableStatement(stmt)) {
        for (const decl of stmt.declarationList.declarations) {
          ...
          if (ts.isIdentifier(decl.name)) {
            const name = _chunk6LLWWLOSjs.getText.call(void 0, decl.name, options);
            exposed[name] = name;
          } else if (ts.isObjectBindingPattern(decl.name)) {
            decl.name.elements.forEach((element) => {
              if (!ts.isIdentifier(element.name)) return;
              exposedValues.push(_chunk6LLWWLOSjs.getText.call(void 0, element.name, options));
              exposed[_chunk6LLWWLOSjs.getText.call(void 0, element.name, options)] = element.propertyName && ts.isIdentifier(element.propertyName) ? _chunk6LLWWLOSjs.getText.call(void 0, element.propertyName, options) : _chunk6LLWWLOSjs.getText.call(void 0, element.name, options);
            });
          }
        }
      } else if (stmt.name && ts.isIdentifier(stmt.name)) {
        const name = _chunk6LLWWLOSjs.getText.call(void 0, stmt.name, options);
        exposed[name] = name;
      }
      _mugglestring.replaceSourceRange.call(void 0, 
        codes,
        "scriptSetup",
        _chunk6LLWWLOSjs.getStart.call(void 0, exportModifier, options),
        exportModifier.end,
  ...
  if (_optionalChain([sfc, 'access', _4 => _4.scriptSetup, 'optionalAccess', _5 => _5.generic])) {
    _chunk6LLWWLOSjs.addCode.call(void 0, codes, `const __VLS_exportExposed = {
`, ...exposedStrings, `};
`);
