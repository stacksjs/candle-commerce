...
function getDefineGenerics(ts, sfc, codes) {
  ...
  ts.forEachChild(sourceFile, (node) => {
    if (ts.isTypeAliasDeclaration(node) && ts.isTypeReferenceNode(node.type) && ts.isIdentifier(node.type.typeName) && node.type.typeName.escapedText === "DefineGeneric") {
      ...
      const typeArgument = _optionalChain([node, 'access', _4 => _4.type, 'access', _5 => _5.typeArguments, 'optionalAccess', _6 => _6[0]]) ? ` extends ${_chunk6LLWWLOSjs.getText.call(void 0, node.type.typeArguments[0], { ts, sfc })}` : "";
      const defaultType = _optionalChain([node, 'access', _7 => _7.type, 'access', _8 => _8.typeArguments, 'optionalAccess', _9 => _9[1]]) ? ` = ${_chunk6LLWWLOSjs.getText.call(void 0, node.type.typeArguments[1], { ts, sfc })}` : "";
